...
- Combine data from different sources using a well-chosen join function in the format `something_join(x, y)`:
  ...
  - `inner_join()`: all rows from `x` where there are matching values in `y`, returning
...
**Q - Which join function might you use when you want a data set with all the rows and the columns in `x` and `y`?**
...
**Q - Which join function might you use when you want a data set with all the columns from `x` and `y` and only the rows existing in both data frames?**
...
- Watch videos for [Prepare: May 19](https://sta199-summer22.netlify.app/prepare/week02_may19_BJ.html)
