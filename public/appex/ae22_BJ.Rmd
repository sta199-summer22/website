...
subtitle: "due Tuesdays, June 14 at 9:29am"
author: "Bora Jin"
editor_options:
...
on the dependent variable. Since Model 1 only includes main effects for age and type, it is called a main effects only model.   
...
**Q - ** What do we assume about relationship between car age and selling price by different car types in Model 1 and Model 2? 

- Model 1: 
- Model 2: 

**Q - ** Use appropriate functions to find the fitted model and display the results in `tidy` format. Hint: `y ~ x1*x2` fits two main effects for `x1` and `x2` and their interaction effect. 
...
### Part 3: CLT-based CI for parameters in a regression framework
...
Using CLT, a $(1-\alpha)*100$% confidence interval for $\beta$ is obtained as 

$$ \hat{\beta} \pm t^*_{n-p} \times \hat{se}$$ 
where $t^*_{n-p}$ is a $1-\alpha/2$ quantile value of a t-distribution with degrees of freedom $n-p$. The number of observations is $n$, and the number of parameters in the model is $p$. For instance, in the above model, $p=3$. 
...
### Part 4: CLT-based HT for parameters in a regression framework
...
- Does constant variance condition hold? 
...
**Q - ** Repeat the same exercise for Model 2.  
