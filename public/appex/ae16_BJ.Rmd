...
- `type = permute`: shuffle the data without replacement
  - for hypothesis testing on a difference in the outcome between groups 
...
- `type = draw`: sample from a theoretical distribution 
  - only for hypothesis testing on a single proportion
...
- `type = bootstrap`: re-sample the original data with replacement
  - for confidence intervals (CI) or for hypothesis testing (HT) on a single mean / median
...
We create new variables for relative change in push-ups and pull-ups before and after the training. 
...
**Q - ** Visualize the rejection region under the null hypothesis with informative labels and compute the p-value. 

```{r part1-pvalue, eval = FALSE}
visualize(null_dist) +
  labs(x = "Difference in sample means of improvement in pull-ups", 
       y = "Count") +
