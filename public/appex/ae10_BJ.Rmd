...
Let's make a table that includes the year, the number of people who prefer each, and the associated probabilities. 
...
Now let's make a table looking at the relationship between favorite animal and favorite movie.

```{r animal-movie}
# code here
```

Using `pivot_wider()`, we'll reformat the data into a **contingency table**, a table frequently used to study the association between two categorical variables. In this contingency table, each row will represent a year, each column will represent a movie, and each cell is the number of students have a particular combination of year and movie.  
...
**Q - ** What is the probability a randomly selected STA199 student is a senior **or** does not pick sci-fi as favorite movie genre? 
