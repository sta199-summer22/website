...
We will continue our investigation of single-family home prices in Minneapolis, Minnesota.
...
Now let’s look at the distribution of price for each community. To do so, we will make a faceted histogram where each facet represents a community and displays the distribution of sales price for that community. Once you have modified the code, remove the option `eval = FALSE` from the code chunk header. 
...
A **density plot** is another option. We just connect the boxes in a histogram with a smooth curve. Fill in the code below to create a desntiy plot of `salesprice`. 
...
1. **Question:** What is `coord_flip()` doing in the code chunk above? Try removing it to see. Does it affect `labs()`? 

2. **Question:** Can you detect any homes sold for unusually high prices? In which community? Does it match what we observed in the faceted histogram above? 
...
Segment the number of homes in each `community` into whether a fireplace exists or not (the variable `fireplace`). We intend to make horizontal bars. 
...
Fill in the blank with an informative y-label. 
...
3. **Question:** Which of the above two visualizations do you prefer? Why? Is this answer always the same?

4. **Solve** There is something wrong with each of the plots below. Run the code for each plot, read the error, then identify and fix the problem.
...
(1) Modify the code outline to create a ridge plot examining the 
