...
1. How many observations (rows) does the dataset `hawaii` have? 
How many variables (columns)? What does each row in the dataset represent?
...
2. Reproduce the following tables. The column "count" represents the number of listings, and "percentage" is the associated percentage. For the final table, you may create a new variable using `ifelse()` for "minimum stay < 30?". 

```{r hw1-ex2, echo=FALSE, out.width = '95%'}
knitr::include_graphics("img/hw1_ex2_table.png")
```
