...
```{fig.margin=T}
A **tibble** is a tweaked, opinionated version of the `R` data frame. The two main differences in the usage of a data frame vs a tibble are printing and subsetting. 
```

The `head()` / `tail()` function returns the first / last six rows of the `flights` data. You can specify the number of rows shown with the second argument in the functions, e.g. `tail(flights, 2)`. 
...
We can also use `select()` with certain characteristics. Here, we select all the variables whose names contain "time". Try `?contains` for more info about other select helpers. 
...
We slice off the bottom 5 rows of the `flights` data. Here `n()` computes the number of rows in the data. 
...
**Q - **The code chunk above should return an NA. What is going wrong? Try to fix it to find the mean departure delay. Hint: Try `summary(flights$dep_delay)`.
