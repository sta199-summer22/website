...
**Q - ** Is random sampling employed? Can we extend findings in the study to a broader population, for instance, mother-child dyads across the US?
...
Let's flip a real coin for 10 times and see how this game would go.
...
**Q - **What's the probability of this outcome if the coin is fair?

More generally, we can define $Y$ to be the number of heads. In other words, $Y = \sum_{i=1}^{n} X_i = X_1 + X_2 + ... X_{n}$ where $n$ is the total number of coin flips. In our case, $n=20$. 
...
We can simulate much more coin flips. The code below simulates 100,000 coin flips where each flip is Bernoulli distributed with $p = 0.5$. Note, Bernoulli(p) $\stackrel{d}{=}$ Binomial(1, p).
...
(2) Plot a histogram of the simulation. What does this show? 
...
(3) What is the probability of seeing at least 15 tails? 
